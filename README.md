...
### ۵. تست بدون سخت‌افزار (شبیه‌ساز ESP32)

اگر به سخت‌افزار دسترسی ندارید، شبیه‌ساز را اجرا کنید (راهنمای کامل در `simulator/README.md`):

```bash
# ترمینال ۱ — شبیه‌ساز روی پورت 8080
python simulator/esp32_sim.py --scenario demo

# در .env آدرس را به شبیه‌ساز اشاره دهید:
...
سناریوها: `demo` (چرخه‌ای از همه آلارم‌ها)، `gas_leak`، `fire`، `intrusion`، `normal`.
برای سوییچ به سخت‌افزار واقعی، فقط `ESP32_BASE_URL` را به IP دستگاه برگردانید.

### ۶. فلش ESP32

به پوشه `esp32/` بروید و طبق `README_ESP32.md` عمل کنید.

## ⚡ Real-time و صدای آلارم

- **Real-time (SSE):** داشبورد از طریق `GET /api/stream` (Server-Sent Events) داده‌ها را
  به‌صورت **زنده** و بدون رفرش دریافت می‌کند. اگر اتصال SSE قطع شود، خودکار به polling سوییچ می‌کند.
- **صدای آلارم:** هر زمان سخت‌افزار آلارم بدهد (`alarm=true`) و آلارم mute نباشد،
  فایل `static/alarm.mp3` به‌صورت loop پخش می‌شود. (به‌خاطر سیاست autoplay مرورگر، صدا با
  اولین کلیک کاربر فعال/unlock می‌شود.) دکمه‌ی mute صدا را قطع می‌کند.
- **آستانه‌ها از سخت‌افزار:** آستانه‌های گاز/دما دیگر در بک‌اند محاسبه نمی‌شوند؛ مستقیماً از
  سخت‌افزار خوانده می‌شوند (`GET /config`) و از داشبورد قابل ویرایش‌اند (با `POST /config`
  روی خود سخت‌افزار نوشته می‌شوند). تصمیم آلارم همیشه از خود سخت‌افزار (`alarm_reasons`) می‌آید.

## 🔌 API های اصلی

| Endpoint | Method | توضیحات |
|----------|--------|---------|
| `/` | GET | داشبورد (نیازمند لاگین) |
| `/login` `/logout` | GET/POST | ورود/خروج ادمین |
| `/api/data` | GET | آخرین داده سنسورها (snapshot) |
| `/api/stream` | GET | **SSE** — جریان زنده داده‌ها |
| `/api/config` | GET/POST | خواندن آستانه‌ها از سخت‌افزار / نوشتن روی سخت‌افزار + تنظیمات اپ |
| `/api/chat` | POST | چت متنی با GPT |
